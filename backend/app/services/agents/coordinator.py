...
class CoordinatorAgent:
    ...
    async def expand_characters(self, character_names: list[str], context: str) -> list[CharacterDescription]:
        """
        Expand brief character names into full descriptions.

        Args:
            character_names: List of character names or brief descriptions
            context: Story context for character development

        Returns:
            List of detailed CharacterDescription objects

        Note:
            This is used internally by plan_story. Can also be called
            separately for character-focused expansions.
        """
        try:
            prompt = f"""You are creating detailed character descriptions for a children's story.

Context: {context}

Characters to develop: {', '.join(character_names)}

For each character, provide:
- Name
- Physical description (appearance, clothing, etc.)
- Personality traits
- Role in the story

Make characters vivid, memorable, and age-appropriate."""

            # This would use structured generation
            # For now, plan_story handles this as part of the overall planning
            raise NotImplementedError("Use plan_story instead")

        except Exception as e:
            logger.error(f"Character expansion failed: {e}")
            raise
